...
static void kscan_595a_scan(struct k_work *work) {
    ...
    if (scan_count == 1) {
        LOG_INF("=== kscan-595a configuration ===");
        ...
        LOG_INF("SENSE pin: port=%p pin=%d flags=0x%x",
                config->sense_gpio.port, config->sense_gpio.pin, config->sense_gpio.dt_flags);
        LOG_INF("HC595A count: %d (%d columns)", config->hc595a_count, num_columns);
...
static int kscan_595a_enable(const struct device *dev) {
    struct kscan_595a_data *data = dev->data;
    LOG_INF("kscan-595a enable called");
...
static int kscan_595a_init(const struct device *dev) {
    ...
    LOG_INF("kscan-595a initialized");
    return 0;
}
...
#define KSCAN_595A_INIT(n)                                                     \
    ...
    static const struct kscan_595a_config kscan_595a_config_##n = {            \
        ...
        .hc595a_count = DT_INST_PROP(n, hc595a_count),                         \
                                                                               ...
                                                                               \
    DEVICE_DT_INST_DEFINE(n, kscan_595a_init, NULL,                            \
                          &kscan_595a_data_##n, &kscan_595a_config_##n,        \
